...
fn parse_session_file_once(
    ...
    for line in BufReader::new(file).lines().map_while(Result::ok) {
        ...
        if first_user_message_title.is_none() {
            first_user_message_title = title_from_user_record(&value);
        }
...
fn persist_session(
    ...
    INSERT INTO sessions (
      ...
      source_bucket, started_at, updated_at, agent_nickname, agent_role,
      latest_plan_type, last_model_id, contains_subagents, created_at, imported_at
    )
...
mod tests {
    ...
    #[test]
    fn parser_uses_first_user_message_when_session_index_has_no_title() {
        ...
        std::fs::write(
            &session_path,
            concat!(
                "{\"timestamp\":\"2026-03-24T00:00:00Z\",\"type\":\"session_meta\",\"payload\":{\"id\":\"11111111-1111-1111-1111-111111111111\"}}\n",
                "{\"timestamp\":\"2026-03-24T00:00:00Z\",\"type\":\"response_item\",\"payload\":{\"type\":\"message\",\"role\":\"user\",\"content\":[{\"type\":\"input_text\",\"text\":\"<environment_context> ignored bootstrap </environment_context>\"}]}}\n",
                "{\"timestamp\":\"2026-03-24T00:00:01Z\",\"type\":\"event_msg\",\"payload\":{\"type\":\"user_message\",\"message\":\"现在看这个 PkuClaw 是什么架构，理解想干什么\"}}\n",
        ...
        assert_eq!(parsed.raw_session.session_id, session_id);
        assert_eq!(
            parsed.raw_session.title.as_deref(),
            Some("现在看这个 PkuClaw 是什么架构，理解想干什么")
        );
    }

    #[test]
    fn parser_prefers_session_index_title_over_user_message_fallback() {
        let directory = tempdir().expect("tempdir");
        let session_path = directory.path().join("sample.jsonl");
        ...
        std::fs::write(
            &session_path,
            concat!(
                "{\"timestamp\":\"2026-03-24T00:00:00Z\",\"type\":\"session_meta\",\"payload\":{\"id\":\"22222222-2222-2222-2222-222222222222\"}}\n",
                "{\"timestamp\":\"2026-03-24T00:00:01Z\",\"type\":\"event_msg\",\"payload\":{\"type\":\"user_message\",\"message\":\"用户首条消息\"}}\n",
                "{\"timestamp\":\"2026-03-24T00:00:02Z\",\"type\":\"turn_context\",\"payload\":{\"model\":\"gpt-5.4\"}}\n",
                "{\"timestamp\":\"2026-03-24T00:00:03Z\",\"type\":\"event_msg\",\"payload\":{\"type\":\"token_count\",\"info\":{\"total_token_usage\":{\"input_tokens\":100,\"cached_input_tokens\":0,\"output_tokens\":25,\"reasoning_output_tokens\":5,\"total_tokens\":125}},\"rate_limits\":{\"plan_type\":\"pro\"}}}\n"
            ),
        )
        .expect("write sample");
        let mut titles = HashMap::new();
        titles.insert(session_id.to_string(), "索引标题".to_string());
        ...
        assert_eq!(parsed.raw_session.title.as_deref(), Some("索引标题"));
    }
    ...
    #[test]
    fn recalculate_all_session_values_applies_pricing_catalog_changes() {
        let directory = tempdir().expect("tempdir");
        let codex_home = directory.path().join("codex-home");
        let sessions_dir = codex_home.join("sessions");
        std::fs::create_dir_all(&sessions_dir).expect("sessions dir");
        ...
        write_session_file(
            &sessions_dir.join("reprice.jsonl"),
            session_id,
            &[("2026-03-24T00:00:02Z", 100_000, 25_000, 40_000, 140_000)],
        );
        ...
        let conn = open_connection(&db_path).expect("open db");
        let original: f64 = conn
            .query_row(
                "SELECT value_usd FROM usage_events WHERE session_id = ?1",
                params![session_id],
                |row| row.get(0),
            )
            .expect("query original value");

        conn.execute(
            "
        UPDATE pricing_catalog
        SET output_price_per_million = output_price_per_million * 2.0
        WHERE model_id = 'gpt-5.4'
        ",
            [],
        )
        .expect("change pricing");
        recalculate_all_session_values(&conn).expect("recalculate");
        ...
        let updated: f64 = conn
            ...
            .expect("query updated value");

        assert!(updated > original);
        assert!((updated - (original + (40_000.0 / 1_000_000.0) * 15.0)).abs() < 1e-9);
    }
